...
embryo <- RunWeightedNeighbors(embryo, dims = 1:30, weight.by = "none", prefix = "wt.none")
embryo <- FindClusters(embryo, graph.name = "wt.none_snn", cluster.name = "wt.none_clusters", resolution = 0.8, verbose = FALSE)
```

### wUMAP (stdev weighting)

```{r embryo-wumap, warning=TRUE, message=TRUE}
embryo <- RunWeightedNeighbors(embryo, dims = 1:30, weight.by = "stdev", prefix = "wt.stdev")
embryo <- FindClusters(embryo, graph.name = "wt.stdev_snn", cluster.name = "wt.stdev_clusters", resolution = 0.8, verbose = FALSE)
```

### UMAP embedding

```{r embryo-umap, warning=TRUE, message=TRUE}
set.seed(42)
embryo <- RunUMAP(embryo, dims = 1:30, reduction.name = "umap")
```

### Visualization

```{r embryo-umap-plot, fig.height=7}
p1 <- DimPlot(embryo, reduction = "umap", group.by = "wt.none_clusters", label = TRUE) +
  ggtitle("wUMAP weight.none (standard)") + NoLegend()
p2 <- DimPlot(embryo, reduction = "umap", group.by = "wt.stdev_clusters", label = TRUE) +
...
data.frame(Method = c("wUMAP weight.none", "wUMAP stdev"),
           ARI = c(ari_none, ari_stdev))
```
